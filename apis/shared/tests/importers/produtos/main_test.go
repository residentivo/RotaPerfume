...
func TestParseAtivo(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.nome, func(t *testing.T) {
			got := produtos.ParseAtivo(tc.in)
			if got != tc.want {
				t.Errorf("produtos.ParseAtivo(%q) = %v, want %v", tc.in, got, tc.want)
			}
		})
	}
}
...
func TestParseDataLancamento(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.nome, func(t *testing.T) {
			got, err := produtos.ParseDataLancamento(tc.in)
			if tc.wantErr {
				if err == nil {
					t.Fatalf("produtos.ParseDataLancamento(%q) esperava erro, obteve nil", tc.in)
				}
				return
			}
			if err != nil {
				t.Fatalf("produtos.ParseDataLancamento(%q) erro inesperado: %v", tc.in, err)
			}
			if tc.want == nil {
				if got != nil {
					t.Errorf("produtos.ParseDataLancamento(%q) = %v, want nil", tc.in, got)
				}
				return
			}
			if got == nil {
				t.Fatalf("produtos.ParseDataLancamento(%q) = nil, want %v", tc.in, tc.want)
			}
			if !got.Equal(*tc.want) {
				t.Errorf("produtos.ParseDataLancamento(%q) = %v, want %v", tc.in, got, tc.want)
			}
		})
	}
}
...
func TestParseRow(t *testing.T) {
	testCases := []struct {
		...
		record  []string
		wantErr bool
		check   func(t *testing.T, row produtos.Row)
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.nome, func(t *testing.T) {
			...
			row, err := produtos.ParseRow(record)
			if tc.wantErr {
				if err == nil {
					t.Fatalf("produtos.ParseRow(%v) esperava erro, obteve nil", tc.record)
				}
				return
			}
			if err != nil {
				t.Fatalf("produtos.ParseRow(%v) erro inesperado: %v", tc.record, err)
			}
			if tc.check != nil {
				tc.check(t, row)
			}
...
func TestResolveCSVPath_FlagTemPrioridade(t *testing.T) {
	got, err := produtos.ResolveCSVPath("/caminho/custom/produtos.csv")
	if err != nil {
