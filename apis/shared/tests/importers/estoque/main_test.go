...
func TestParseRuptura(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.nome, func(t *testing.T) {
			got := estoque.ParseRuptura(tc.in)
			if got != tc.want {
				t.Errorf("estoque.ParseRuptura(%q) = %v, want %v", tc.in, got, tc.want)
			}
		})
	}
}
...
func TestParseRow(t *testing.T) {
	testCases := []struct {
		...
		record  []string
		wantErr bool
		check   func(t *testing.T, row estoque.Row)
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.nome, func(t *testing.T) {
			...
			row, err := estoque.ParseRow(record)
			if tc.wantErr {
				if err == nil {
					t.Fatalf("estoque.ParseRow(%v) esperava erro, obteve nil", tc.record)
				}
				return
			}
			if err != nil {
				t.Fatalf("estoque.ParseRow(%v) erro inesperado: %v", tc.record, err)
			}
			if tc.check != nil {
				tc.check(t, row)
			}
...
func TestResolveCSVPath_FlagTemPrioridade(t *testing.T) {
	got, err := estoque.ResolveCSVPath("/caminho/custom/estoque.csv")
	if err != nil {
...
func TestUpsertAll_InsertNovoAtualizaExistenteEContaErros(t *testing.T) {
	...
	inserted, updated, failed, err := estoque.UpsertAll(db, rows)
	if err != nil {
